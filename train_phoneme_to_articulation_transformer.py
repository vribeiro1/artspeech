####################################################################################################
#
# Train the model-free phoneme-to-articulation using a transformer-based network
#
...
def run_epoch(
    ...
    for (
        ...
        src_attn_mask = src_attn_mask.to(device)
        tgt_attn_mask = tgt_attn_mask.to(device)

        bs, seq_len, channels, _, features = targets.shape
...
if __name__ == "__main__":
    ...
    parser.add_argument("--experiment", dest="experiment_name", default="transformer_based")
